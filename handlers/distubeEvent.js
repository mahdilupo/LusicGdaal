console.log(`Welcome to SERVICE HANDLER /--/ By http://discord.io/lupo /--/ Discord: Lupo#1725`.yellow);
...
module.exports = (client) => {
  ...
  client.on(`interactionCreate`, async (interaction) => {
    ...
    if (interaction.isSelectMenu()) {
      let link = `https://open.spotify.com/playlist/37i9dQZF1E36WWDQ6fu2XT?si=UfFTXSjuSqmg0E6fMFpmig&utm`;
      if (interaction.values[0]) {
        //ncs | no copyrighted music
        if (interaction.values[0].toLowerCase().startsWith(`n`)) link = `https://open.spotify.com/playlist/7sZbq8QGyMnhKPcLJvCUFD`;
        //persian Hip-Hop
        if (interaction.values[0].toLowerCase().startsWith(`p`)) link = `https://open.spotify.com/playlist/37i9dQZF1E36WWDQ6fu2XT?si=UfFTXSjuSqmg0E6fMFpmig&utm`;
        //default
        if (interaction.values[0].toLowerCase().startsWith(`d`)) link = `https://open.spotify.com/playlist/37i9dQZF1E36WWDQ6fu2XT?si=UfFTXSjuSqmg0E6fMFpmig&utm`;
  ...
  function generateQueueEmbed(client, guildId, leave) {
    ...
    if (!leave && newQueue && newQueue.songs[0]) {
      ...
      .setColor(ee.color)
      .setDescription(String(songs.map((track, index) => `**\` ${++index}. \` ${track.url ? `[${track.name.substr(0, 60).replace(/\[/igu, `\\[`).replace(/\]/igu, `\\]`)}](${track.url})` : track.name}** - \`${track.isStream ? `LIVE STREAM` : track.formattedDuration}\``).join(`\n`)).substr(0, 2048));
    ...
    var stopbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Stop').setEmoji(`⏹️`).setLabel(`Stop`)
    var skipbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Skip').setEmoji(`⏭`).setLabel(`Skip`)
    var shufflebutton = new MessageButton().setStyle('SECONDARY').setCustomId('Shuffle').setEmoji('🔀').setLabel(`Shuffle`)
    var pausebutton = new MessageButton().setStyle('SECONDARY').setCustomId('Pause').setEmoji('⏸').setLabel(`Pause`)
    var autoplaybutton = new MessageButton().setStyle('SECONDARY').setCustomId('Autoplay').setEmoji('🔁').setLabel(`Autoplay`)
    var songbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Song').setEmoji(`🔁`).setLabel(`Song`)
    var queuebutton = new MessageButton().setStyle('SECONDARY').setCustomId('Queue').setEmoji(`🔂`).setLabel(`Queue`)
    var forwardbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Forward').setEmoji('⏩').setLabel(`+10 Sec`)
    var rewindbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Rewind').setEmoji('⏪').setLabel(`replay`)
    var lyricsbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Lyrics').setEmoji('📝').setLabel(`Lyrics`)
    if (!leave && newQueue && newQueue.songs[0]) {
      ...
      if (newQueue.paused) {
        pausebutton = pausebutton.setStyle('SECONDARY').setEmoji('▶️').setLabel(`Resume`)
      }
  ...
  function receiveQueueData(newQueue, newTrack) {
    ...
    let skip = new MessageButton().setStyle('SECONDARY').setCustomId('1').setEmoji(`⏭`).setLabel(`Skip`)
    let stop = new MessageButton().setStyle('SECONDARY').setCustomId('2').setEmoji(`⏹️`).setLabel(`Stop`)
    let pause = new MessageButton().setStyle('SECONDARY').setCustomId('3').setEmoji('⏸').setLabel(`Pause`)
    let autoplay = new MessageButton().setStyle('SECONDARY').setCustomId('4').setEmoji('🔁').setLabel(`Autoplay`)
    let shuffle = new MessageButton().setStyle('SECONDARY').setCustomId('5').setEmoji('🔀').setLabel(`Shuffle`)
    if (!newQueue.playing) {
    ...
    let songloop = new MessageButton().setStyle('SECONDARY').setCustomId('6').setEmoji(`🔁`).setLabel(`Song`)
    let queueloop = new MessageButton().setStyle('SECONDARY').setCustomId('7').setEmoji(`🔂`).setLabel(`Queue`)
    let forward = new MessageButton().setStyle('SECONDARY').setCustomId('8').setEmoji('⏩').setLabel(`+10 Sec`)
    let rewind = new MessageButton().setStyle('SECONDARY').setCustomId('9').setEmoji('⏪').setLabel(`replay`)
    let lyrics = new MessageButton().setStyle('SECONDARY').setCustomId('10').setEmoji('📝').setLabel(`Lyrics`).setDisabled();
